...
This shows that `surveys` is an object called a **tibble**, which is tidyverse's way of dealing with tables. What we need to know about this at the moment is that it prints things a bit nicer on our screens. Note that only the first few rows of the `surveys` data frame were printed (we would not like to see 30,000+ rows printed on our screen!).
